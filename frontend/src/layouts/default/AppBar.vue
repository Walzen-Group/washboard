    ...
    <v-app-bar elevation="1" density="compact" color="washboard-appbar">
        ...
        <v-tooltip v-if="smAndUp" location="bottom">
            ...
            <span v-if="updateQuelelelStore.queueCount > 0">{{
                Object.keys(updateQuelelelStore.queue[QueueStatus.Queued]).join(", ")
            }}</span>
        ...
        <template v-slot:append>
            <div class="ma-2 d-flex align-center">
                <v-btn
                    rounded
                    @click="signOut"
                    :size="miniVariant ? 39 : 'default'"
                    variant="tonal"
                    :icon="miniVariant"
                    class="mx-auto"
                >
                    <v-fade-transition hide-on-leave>
                        <span v-if="!miniVariant"> Sign Out </span>
                        <v-icon class="pl-1" v-if="miniVariant" :size="24"> mdi-logout </v-icon>
                    </v-fade-transition>
